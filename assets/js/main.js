// main.js (modified to include customer name input)
...
// Generate a product card. Added a text input to capture customer name.
function createProductCard({ name, price, mixes, promo }) {
    ...
    mixes && mixes.length
      ? mixes
        ...
        <label class="mix-option">
          <input type="radio" name="mix-${name}" value="${m.label}" data-price="${m.price}" ${
              i === 0 ? 'checked' : ''
            }> ${m.label}
        </label>`;
          })
          .join('')
      : `<input type="hidden" name="mix-${name}" value="ไม่มี">`;
  ...
  card.innerHTML = `
    ...
    <!-- NEW: customer name input below quantity control -->
    <input type="text" class="customer-name-input" placeholder="ชื่อลูกค้า (ไม่บังคับ)">
